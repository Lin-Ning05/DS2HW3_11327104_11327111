...
class Hash { //以線性為基礎款 
  ...
  public:
    ...
    void Search(std::string toSearch) {
        char target[10];
        strcpy(target , toSearch.c_str());
        int initial = HashFunction(target);
        int index = initial;
        int compare = 1;

        int n = 1;

        while (hashingTable[index].used && strcmp(hashingTable[index].sid, target) != 0) {
            index = (initial + Step(n, target)) % tableSize;
            n++;
            compare++;
            if (n == tableSize) {
                std::cout << toSearch << " is not found after " << compare << " probes.\n" << std::endl;
                return; //無窮迴圈
            }
        }
...
void Hash::WriteInfo(std::string outputName) {
    ...
    for (int i = 0 ; i < tableSize ; i++) {
        ...
        fout << std::setw(11) << hashingTable[i].hvalue << ", " << std::setw(10) << hashingTable[i].sid << ", " <<
                std::setw(10) << hashingTable[i].sname << ", " << std::setw(10) << hashingTable[i].average << std::endl;
    }
    fout.close();
}
