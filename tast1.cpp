...
class Hash { //已線性為基礎款
  protected:
    ...
    virtual int Step(int n) {//計算要加幾格 覆寫
        return n;
    }
...
class QuadraticHash : public Hash {
  private:
    ...
    int Step(int n) override { //複寫線性的版本，此處是N平方
...
void Hash::WriteInfo(std::string outputName) {
    ...
    for (int i = 0 ; i < tableSize ; i++) {
        ...
        fout << std::setw(11) << hashingTable[i].hvalue << "," << std::setw(11) << hashingTable[i].sid << "," << 
                std::setw(8) << hashingTable[i].sname << "," << std::setw(11) << hashingTable[i].average << std::endl;
    }
    fout.close();
}
...
float Hash::CountExist(std::vector<HashingNode>& HashingInfo) {
    ...
    for (int i = 0; i < HashingInfo.size(); i++) {
        ...
        while (strcmp(hashingTable[index].sid, target.sid) != 0) {
            if (!hashingTable[index].used) break;
            index = (initial + Step(n)) % tableSize;
            n++;
            compare++;
        }
...
void QuadraticHash::Create(std::vector<HashingNode> HashingInfo) {
    std::cout << "Hash table has been successfully created by Quadratic probing" << std::endl;
    Insert(HashingInfo);
