...
class Hash { //以線性為基礎款
  protected:
    ...
    int datasize;
    virtual void WriteInfo(std::string outputName);
  ...
  public:
    ...
    Hash() {}
    virtual void Insert(std::vector<HashingNode> HashingInfo);
};
...
class BucketsHash : public Hash {
  private:
    std::vector<std::vector<HashingNode>> hashingTable;
    bool Collision(int index) override {//永遠沒有碰撞的可能
        for (int i = 0; i < 2; i++) {
            if (!hashingTable[index][i].used) {
                return false;
            }
        }
        return true;
    }
    void Insert(std::vector<HashingNode> HashingInfo)override;
    void WriteInfo(std::string outputName)override;
  public:
    BucketsHash(int infoNum) {
        datasize = infoNum;
        tableSize = BtPrime(infoNum * 1.15);
        hashingTable.resize(tableSize, std::vector<HashingNode>(2));
...
int main() {
    ...
    while(true) {
        ...
        if (verb == "0") break;
            ...
            BucketsHash bucketsHash = BucketsHash (HashingInfo.size());
            bucketsHash.Create(HashingInfo);
            bucketsHash.WriteFile(num);
        }
...
void Hash::Insert(std::vector<HashingNode> HashingInfo) {
    for (int i = 0 ; i < HashingInfo.size() ; i++) {
        ...
        if (hashingTable[index].used == true) {// 鏈結串列
...
void BucketsHash::Create(std::vector<HashingNode> HashingInfo) {
    std::cout << "\nHash table has been successfully created by BucketsHash   " << std::endl;
    Insert(HashingInfo);
...
void BucketsHash::WriteFile(std::string num) {
    std::string output = "BucketsHash" + num + ".txt";
    std::ofstream fout1(output);
    fout1 << " --- Hash table created by BucketsHash    ---" << std::endl;
    fout1.close();
    WriteInfo(output);
    std::ofstream fout2(output, std::ios::app);
    fout2 << " ----------------------------------------------------- " << std::endl;
...
void BucketsHash::Insert(std::vector<HashingNode> HashingInfo) {
    for (int i = 0 ; i < HashingInfo.size() ; i++) {
        HashingNode toInsert = HashingInfo[i];
        int index = HashFunction(toInsert.sid);
        toInsert.hvalue = index;
        ...
        for (int i = 0; i < 2; i++) {
            if (!hashingTable[index][i].used) {
                hashingTable[index][i] = toInsert;
                hashingTable[index][i].used = true;
                hashingTable[index][i].next = nullptr;
                break;
            }
        }
    }
...
void BucketsHash::WriteInfo(std::string outputName) {
    std::ofstream fout(outputName, std::ios::app); // 接在後面
    for (int i = 0 ; i < tableSize ; i++) {
        fout << "[" << std::setw(3) << i << "]";
        if (!hashingTable[i][0].used) {
            fout << " " << std::endl;
            continue;
        }
        for (int j = 0; j < 2; j++) {
            if (hashingTable[i][j].used) {
                fout << std::setw(11) << hashingTable[i][j].hvalue << ", " << std::setw(10) << hashingTable[i][j].sid << ", " <<
                    std::setw(10) << hashingTable[i][j].sname << ", " << std::setw(10) << hashingTable[i][j].average << std::endl;
            }else break;
        }    
    }
    fout.close();
